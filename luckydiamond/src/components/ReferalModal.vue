<template>
  <div class="payment-modal withdraw-modal">
    ...
    <div class="payments-modal__deposit">
      ...
      <div class="deposit-input deposit-text referal-icon-diamond deposit-icon__input">
        <br>
        <h3 v-if="referralData.referralCode">{{ referralData.referralCode }}</h3>
        <input :class="{ 'animate-start-btn': errorDeposit }" readonly class="deposit-amount__input" v-model="amount" type="number" />
        <br>
        <br>
        ...
        <div class="deposit-btns">
          <ul class="display-btns btns-style-diamonds">
            <li v-for="(number, index) in PaymentsModalNumbers" :key="index">
              <button @click="clickedBtnChoice(index, number.diamonds)" :class="{ 'btn-click': clickedBtn === index, [number]: clickedBtn === number }">{{ number.diamonds }}</button>
            </li>
          </ul>
        </div>
